        ...
        <div className="lg:w-8/12 bg lg:mx-auto mb-8">
          <header className="flex flex-wrap items-center p-4 md:py-8">
            ...
            <div className="w-8/12 md:w-7/12 ml-4">
              ...
              {user.userDetails.church && <span className="flex items-center gap-1"><BiChurch size={20}/>{user.userDetails.church}</span>}
              {user.userDetails.branch && <span className="flex items-center gap-1"><GiFamilyTree size={20}/>{user.userDetails.branch}</span>}
              {user.userDetails.location && <span className="flex items-center gap-1"><GrLocation size={20}/>{user.userDetails.location}</span>}
              {user.userDetails.instagram && <span className="flex items-center gap-1"><FaInstagram size={20}/>{`@${user.userDetails.instagram}`}</span>}
              {user.userDetails.number && <span className="flex items-center gap-1"><AiOutlineWhatsApp size={20}/>{user.userDetails.number}</span>}
              {user.userDetails.bio && <p className="mt-2"><span className="font-medium ">Bio:</span> {user.userDetails.bio}</p>}
              </div>

            {/* <!-- user meta form small screens --> */}
            <div className="md:hidden space-y-1 text-sm my-2">
            <h1 className="font-semibold">{user.userDetails.fullname}</h1>
              {user.userDetails.church && <span className="flex items-center gap-1"><BiChurch size={20}/>{user.userDetails.church}</span>}
              {user.userDetails.branch && <span className="flex items-center gap-1"><GiFamilyTree size={20}/>{user.userDetails.branch}</span>}
              {user.userDetails.location && <span className="flex items-center gap-1"><GrLocation size={20}/>{user.userDetails.location}</span>}
