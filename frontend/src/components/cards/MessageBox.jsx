...
const MessageCard = ({ message, own }) => {
  const messageStyle = {
    myMessage:
      "px-6 py-3 rounded-t-full rounded-l-full text-white bg-blue-500 max-w-xs lg:max-w-md",
    personMessage:
      "px-6 py-3 rounded-t-full rounded-r-full bg-gray-100 max-w-xs lg:max-w-md",
  };
  return (
    <div className="flex flex-col items-start group">
      <div
        className={`flex  w-full items-center ${
          own ? "justify-start flex-row-reverse" : "justify-start"
        }`}
      >
        <p
          className={own ? messageStyle.myMessage : messageStyle.personMessage}
        >
          {message.text}
        </p>
